...
bool Console::init(uint8_t cells_x, uint8_t cells_y) {
	...
	// First, save the previous mode to restore at exit.
	GetConsoleMode(StdIn, &dwOldInputMode);

	// Enable mouse input and window input and disable quick-edit mode (for windows 10).
	dwInputMode = ENABLE_WINDOW_INPUT | ENABLE_MOUSE_INPUT | ENABLE_EXTENDED_FLAGS | (dwOldInputMode & ~ENABLE_QUICK_EDIT_MODE);
